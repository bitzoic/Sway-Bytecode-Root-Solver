...
fn main(identity: Identity, bytecode: Vec<u8>, config: Vec<(u64, Vec<u8>)>) -> b256 {
    // Swap the configurable values and compute the bytecode root
    let result_bytecode = swap_configurables(bytecode, config);
    let result_bytecode_root = compute_bytecode_root(result_bytecode);

    match identity {
        Identity::ContractId(id) => {
            // Fetch the bytecode root of the deployed contract
            let root = bytecode_root(id);
        ...
        Identity::Address(address) => {
            ...
            hasher.write(seed);
            hasher.write(Bytes::from(result_bytecode_root));
...
// Swaps out configurables in some bytecode
fn swap_configurables(bytecode: Vec<u8>, config: Vec<(u64, Vec<u8>)>) -> Vec<u8> {
...
// Takes some bytes and creates a leaf
pub fn leaf_digest(data: raw_slice) -> b256 {
...
// Takes two nodes and creates a node
